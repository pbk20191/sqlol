...
import io.roastedroot.sqlite4j.core.SafeStmtPtr.*
import io.roastedroot.sqlite4j.date.FastDateFormat.Companion.getInstance
...
protected constructor(stmt: CoreStatement) : CoreResultSet(stmt) {
    ...
    fun next(): Boolean {
        ...
        // do the real work
        val statusCode =
            stmt.pointer!!.safeRunInt<SQLException>(SafePtrIntFunction { obj: DB?, stmt: Long -> obj!!.step(stmt) })
        when (statusCode) {
            Codes.SQLITE_DONE -> {
                pastLastRow = true
                return false
    ...
    fun getBytes(col: Int): ByteArray? {
        return stmt.pointer!!.safeRun<ByteArray?, SQLException>(SafePtrFunction { db: DB?, ptr: Long ->
            db!!.column_blob(
                ptr,
                markCol(col)
            )
    ...
    fun getDate(col: Int): Date? {
        when (safeGetColumnType(markCol(col))) {
            ...
            Codes.SQLITE_TEXT -> {
                ...
                return Date(julianDateToCalendar(safeGetDoubleCol(col))!!.getTimeInMillis())
            }
    ...
    fun getDate(col: Int, cal: Calendar): Date? {
        ...
        when (safeGetColumnType(markCol(col))) {
            ...
            Codes.SQLITE_TEXT -> {
                ...
                return Date(julianDateToCalendar(safeGetDoubleCol(col), cal)!!.getTimeInMillis())
            }
    ...
    fun getInt(col: Int): Int {
        return stmt.pointer!!.safeRunInt<SQLException>(SafePtrIntFunction { db: DB?, ptr: Long ->
            db!!.column_int(
                ptr,
                markCol(col)
            )
    ...
    fun getTime(col: Int): Time? {
        when (safeGetColumnType(markCol(col))) {
            ...
            Codes.SQLITE_TEXT -> {
                ...
                return Time(julianDateToCalendar(safeGetDoubleCol(col))!!.getTimeInMillis())
            }
    ...
    fun getTime(col: Int, cal: Calendar): Time? {
        ...
        when (safeGetColumnType(markCol(col))) {
            ...
            Codes.SQLITE_TEXT -> {
                ...
                return Time(julianDateToCalendar(safeGetDoubleCol(col), cal)!!.getTimeInMillis())
            }
    ...
    fun getTimestamp(col: Int): Timestamp? {
        when (safeGetColumnType(markCol(col))) {
            ...
            Codes.SQLITE_TEXT -> {
                ...
                return Timestamp(julianDateToCalendar(safeGetDoubleCol(col))!!.getTimeInMillis())
            }
    ...
    private fun getColumnDeclType(col: Int): String? {
        var declType: String? = stmt.pointer!!.safeRun<String?, SQLException>(SafePtrFunction { db: DB?, ptr: Long ->
            db!!.column_decltype(
                ptr,
                checkCol(col)
            )
    ...
    private fun safeGetLongCol(col: Int): Long {
        return stmt.pointer!!.safeRunLong<SQLException>(SafePtrLongFunction { db: DB?, ptr: Long ->
            db!!.column_long(
                ptr,
                markCol(col)
            )
    ...
    private fun safeGetDoubleCol(col: Int): Double {
        return stmt.pointer!!.safeRunDouble<SQLException>(SafePtrDoubleFunction { db: DB?, ptr: Long ->
            db!!.column_double(
                ptr,
                markCol(col)
            )
    ...
    private fun safeGetColumnText(col: Int): String? {
        return stmt.pointer!!.safeRun<String?, SQLException>(SafePtrFunction { db: DB?, ptr: Long ->
            db!!.column_text(
                ptr,
                markCol(col)
            )
